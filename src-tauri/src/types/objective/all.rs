...
impl Objective {
    ...
    pub async fn all(wipe_id: i64, db_lock: Arc<Mutex<Connection>>) -> Vec<Self> {
        ...
        let prep = db.prepare(
                "SELECT id, description, optional, count, found_in_raid, item, task, completed, wipe, collected, dogtag_level, min_durability, max_durability FROM objectives WHERE wipe = ?",
            );
        if prep.is_err() {
